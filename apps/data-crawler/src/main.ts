...
const NOTE_ABOUT_SOURCE = `// This file is generated by apps/data-crawler/src/main.ts`;
...
const main = async () => {
  ...
  const dataWithUniqueTitles = data?.filter(
    (item, index, array) => item && array.findIndex((i) => i?.title === item?.title) === index,
  );
  const dataToSave = JSON.stringify(dataWithUniqueTitles, null, 2);
